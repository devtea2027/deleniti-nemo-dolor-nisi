# @devtea2027/deleniti-nemo-dolor-nisi
[![license](https://img.shields.io/badge/license-MIT-brightgreen)](LICENSE)
[![npm version](https://img.shields.io/npm/v/@devtea2027/deleniti-nemo-dolor-nisi.svg?style=flat-square)](https://www.npmjs.org/package/@devtea2027/deleniti-nemo-dolor-nisi)
...
```JavaScript
import { EncoderStream } from '@devtea2027/deleniti-nemo-dolor-nisi';
...
```JavaScript
import { encodeAsIterable } from '@devtea2027/deleniti-nemo-dolor-nisi';
...
```JavaScript
import { encodeAsAsyncIterable } from '@devtea2027/deleniti-nemo-dolor-nisi';
...
```HTML
<script src="node_modules/@devtea2027/deleniti-nemo-dolor-nisi/dist/index.js"></script>
```
...
```JavaScript
import { Encoder } from '@devtea2027/deleniti-nemo-dolor-nisi';
let encoder = new Encoder();
encoder.encode(myBigData);
```

Another way to further leverage the benefits of the @devtea2027/deleniti-nemo-dolor-nisi record structures is to use streams that naturally allow for data to reuse based on previous record structures. The stream classes have the record structure extension enabled by default and provide excellent out-of-the-box performance.
...
Another useful way of using @devtea2027/deleniti-nemo-dolor-nisi, and the record extension, is for storing data in a databases, files, or other storage systems. If a number of objects with common data structures are being stored, a shared structure can be used to greatly improve data storage and deserialization efficiency. In the simplest form, provide a `structures` array, which is updated if any new object structure is encountered:
...
* `useFloat32` - This will enable @devtea2027/deleniti-nemo-dolor-nisi to encode non-integer numbers as 32-bit (4 byte) floating point numbers. See next section for possible values.
* `alwaysUseFloat` - This will force @devtea2027/deleniti-nemo-dolor-nisi to encode any number, including integers, as floating-point numbers.
...
```JavaScript
import { ALWAYS, DECIMAL_ROUND, DECIMAL_FIT } from '@devtea2027/deleniti-nemo-dolor-nisi'
```
...
require("@devtea2027/deleniti-nemo-dolor-nisi").encode(obj);                             |  163100 |  5001 |  32613
require("@devtea2027/deleniti-nemo-dolor-nisi").decode(buf);                             |  100200 |  5004 |  20023
@devtea2027/deleniti-nemo-dolor-nisi w/ shared structures: packr.encode(obj);            |  178300 |  5002 |  35645
@devtea2027/deleniti-nemo-dolor-nisi w/ shared structures: packr.decode(buf);            |  414000 |  5000 |  82800
...
(`avsc` is schema-based and more comparable in style to @devtea2027/deleniti-nemo-dolor-nisi with shared structures).

Here is a benchmark of streaming data (again borrowed from `msgpack-lite`'s benchmarking), where @devtea2027/deleniti-nemo-dolor-nisi is able to take advantage of the structured record extension and really demonstrate its performance capabilities:
...
## Custom Extensions
You can add your own custom extensions, which can be used to encode specific types/classes in certain ways. This is done by using the `addExtension` function, and specifying the class, extension type code (custom extensions should be a number greater than 40500, all others are reserved for  CBOR or @devtea2027/deleniti-nemo-dolor-nisi), and your encode and decode functions (or just the one you need). You can use @devtea2027/deleniti-nemo-dolor-nisi encoding and decoding within your extensions:
```JavaScript
import { addExtension, Encoder } from '@devtea2027/deleniti-nemo-dolor-nisi';
...
### Structured Cloning
With structured cloning enabled, @devtea2027/deleniti-nemo-dolor-nisi will also use tags/extensions to store Set, Map, Error, RegExp, ArrayBufferView objects and preserve their types.
